...
Move AI::get_best_move(Board b, int recursion_depth) {
    ...
    for (auto move = moves.begin(); move != moves.end(); move++) {
        ...
        int score = get_best_score(simulated, recursion_depth - 1);
...
int AI::get_best_score(Board b, int depth) {
    if (depth == 0) {
    ...
    for (auto move = moves.begin(); move != moves.end(); move++) {
        Board simulated = b.copy();
        simulated.exec_move(*move);
        int testing_score = get_best_score(b, depth - 1);
